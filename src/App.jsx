...
export default function App() {
  ...
  const [userId, setUserId] = useState(() => localStorage.getItem("pulsewatch_user") || "");
  ...
  useEffect(() => {
    if (token) {
      ...
      localStorage.setItem("pulsewatch_user", userId);
    } else {
    ...
    try {
      ...
      setAuthError(
        err instanceof TypeError
          ? "Can't reach the PulseWatch server. Is the backend running on :8000?"
          : err.message
      );
    } finally {
        ...
        <div className="w-full max-w-[380px]">
          <div className="flex items-center gap-2 justify-center mb-8">
            <div className="h-9 w-9 rounded-lg bg-ink flex items-center justify-center">
              <span className="text-brand font-extrabold text-lg leading-none">P</span>
            </div>
            <span className="text-xl font-bold tracking-tight text-ink">PulseWatch</span>
          </div>

          <form onSubmit={handleLogin} className="bg-white border border-black/5 rounded-2xl shadow-card p-7">
            ...
            <input
              ...
              className="w-full rounded-lg bg-paper border border-black/10 text-ink text-[14px] px-3.5 py-2.5 mb-4 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 transition-shadow"
            />
